 ...
 */
export const executarGrupo = async (ctx) => {
    const { conexao, origem, argumentos, ehGrupo, ehCriador, ehAdmins, ehBotAdmins, marca, prefixo, responder, metadadosGrupo, downloadContentFromMessage } = ctx;
    if (!ehGrupo) {
    ...
    switch (acao) {
        case 'nome': {
            if (!texto)
                return responder(`${marca} Forneça o novo nome: ${prefixo}grupo nome <texto>`);
        ...
        case 'promover': {
            ...
            if (!usuarios.length)
                return responder(`${marca} Marque os usuários que deseja promover.`);
        ...
        case 'rebaixar': {
            const usuarios = ctx.info?.message?.extendedTextMessage?.contextInfo?.mentionedJid || [];
            if (!usuarios.length)
                return responder(`${marca} Marque os usuários que deseja rebaixar.`);
        ...
        default: {
            await responder(`*Gerenciamento de Grupo*\n\n` +
