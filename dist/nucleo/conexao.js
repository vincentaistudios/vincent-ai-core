...
import { marca, configuracoesBot, obterDiretorioSessao, hora, data } from './contexto.js';
...
console.info = function (...args) {
    ...
    if (mensagem.includes("Closing session: SessionEntry"))
        return;
    if (mensagem.includes("Removing old closed session: SessionEntry {}"))
        return;
    return originalConsoleInfo.apply(console, args);
};
let interfaceLeitura = null;
/**
 * Garante que a interface de leitura do terminal esteja disponível para interação.
 */
function garantirInterfaceLeitura() {
    if (!interfaceLeitura) {
 ...
 */
const perguntar = (texto) => new Promise((resolver) => {
 ...
 */
async function solicitarCodigoPareamentoComTentativa(conexao, numero) {
    const tentativas = 3;
...
async function iniciarRobo() {
    ...
    if ((opcoesCliCodigo || opcoesCliQr) && process.env.MANTER_SESSAO !== "1") {
        try {
            fs.rmSync(diretorioSessao, { recursive: true, force: true });
        }
        catch { }
    ...
    const versaoEstavel = [2, 3000, 1034740716];
    ...
    if (!jaAutenticado) {
        if (preferenciasPareamento.metodo === 'code' || opcoesCliCodigo)
            usarCodigo = true;
        else if (preferenciasPareamento.metodo === 'qr' || opcoesCliQr)
            usarCodigo = false;
        else {
    ...
    conexao.ev.process(async (eventos) => {
        if (eventos['connection.update']) {
            ...
            if (connection === 'close') {
                const codigoStatus = lastDisconnect?.error?.output?.statusCode || DisconnectReason.loggedOut;
