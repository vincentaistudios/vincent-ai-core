 ...
 */
export function formatarTempoHms(segundos) {
    const formatarDígito = (s) => (s < 10 ? '0' : '') + s;
 ...
 */
export const tempoAtividade = function (segundos) {
...
const comandosUsadosRecentemente = new Set();
export const estaFiltrado = (de) => !!comandosUsadosRecentemente.has(de);
export const adicionarFiltro = (de) => {
 ...
 */
export const gerenciadorReconexaoSessao = async (horaAtual, upsert) => {
    switch (horaAtual) {
        case '07:00:00':
        case '12:00:00':
        case '18:00:00':
    ...
    if (upsert?.messages === undefined)
        return;
};
export { fs, logger, chalk, mimetype, moment, encodeUrl, os, util, exec, NodeCache, Boom, colors, path };
