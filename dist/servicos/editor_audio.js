...
function gerarCaminhoTemp(extensao = 'mp3') {
    garantirDirTemp();
    const id = Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
    return path.join(DIR_TEMP, `audio_${id}.${extensao}`);
}
function limparTemp(caminho) {
    try {
...
async function obterDuracaoAudio(caminho) {
    try {
...
export async function cortarAudio(bufferAudio, inicio, fim) {
    const start = parsearTempo(inicio);
    const end = parsearTempo(fim);
    if (start === null || end === null)
        return { ok: false, erro: 'formato_tempo_invalido' };
    if (start < 0 || end < 0 || start >= end)
        return { ok: false, erro: 'tempo_invalido' };
    if ((end - start) > CONFIG.DURACAO_MAXIMA)
    ...
    try {
        ...
        if (duracao && end > duracao) {
            limparTemp(entrada);
            return { ok: false, erro: 'tempo_excede_duracao' };
        }
        ...
        limparTemp(entrada);
        limparTemp(saida);
        return { ok: false, erro: 'falha_ffmpeg' };
    }
}
/** Altera a velocidade do áudio (0.5x a 3.0x) */
export async function alterarVelocidade(bufferAudio, velocidade) {
...
export async function reverterAudio(bufferAudio) {
    const entrada = gerarCaminhoTemp('input');
    const saida = gerarCaminhoTemp('mp3');
    try {
        fs.writeFileSync(entrada, bufferAudio);
...
export async function bassBoost(bufferAudio, ganho = 10) {
...
export async function normalizarVolume(bufferAudio) {
    const entrada = gerarCaminhoTemp('input');
    const saida = gerarCaminhoTemp('mp3');
    try {
        fs.writeFileSync(entrada, bufferAudio);
