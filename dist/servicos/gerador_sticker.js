...
async function converterParaWebp(bufferMedia, ehVideo = false, forcarQuadrado = false) {
    ...
    for (let tentativas = 0; tentativas < 8; tentativas++) {
        ...
        await new Promise((resolve, reject) => {
            ffmpeg(tmpEntrada).outputOptions(opcoes).format('webp')
        ...
        if (bufferSaida.length <= TAMANHO_MAX)
            break;
...
async function escreverExif(webpBuffer, metadata) {
    try {
...
async function resolverParaBuffer(entrada) {
    if (Buffer.isBuffer(entrada))
        return entrada;
    if (typeof entrada === 'string') {
        if (/^data:.*?;base64,/i.test(entrada))
            return Buffer.from(entrada.split(',')[1], 'base64');
        if (/^https?:\/\//i.test(entrada))
            return obterBuffer(entrada);
 ...
 */
export async function enviarSticker(conexao, jid, opcoes, { quoted } = {}) {
    ...
    if (packname || author)
        webpBuffer = await escreverExif(webpBuffer, { packname, author });
