...
async function baixarBuffer(urlArquivo) {
...
async function chamarAPI(endpoint, params) {
    ...
    if (!config)
        return { ok: false, erro: 'api_nao_configurada', mensagem: '⚠️ A API de downloads não está configurada.\nAdicione *DOWNLOADER_API_URL* e *DOWNLOADER_API_KEY* no arquivo .env.' };
    try {
...
export async function youtubeMP4(url) {
...
export async function instagramDL(url) {
    ...
    if (!dlurl)
        return { ok: false, erro: 'url_nao_encontrada', mensagem: '❌ Não foi possível obter o link do Instagram.' };
    const buffer = await baixarBuffer(dlurl);
...
export async function tiktokDL(url) {
    ...
    if (!dlurl)
        return { ok: false, erro: 'url_nao_encontrada', mensagem: '❌ Não foi possível obter o vídeo do TikTok.' };
    const buffer = await baixarBuffer(dlurl);
    return { ok: true, buffer, titulo: res.data.resposta.title || 'TikTok' };
}
/** Baixa vídeo do Kwai */
export async function kwaiDL(url) {
    const res = await chamarAPI('/api/downloads/kwai', { query: url });
    if (!res.ok)
        return res;
    const dlurl = res.data?.resposta?.dlurl;
    if (!dlurl)
        return { ok: false, erro: 'url_nao_encontrada', mensagem: '❌ Não foi possível obter o vídeo do Kwai.' };
    const buffer = await baixarBuffer(dlurl);
...
export async function spotifyDL(url) {
    ...
    const dlurl = res.data?.resposta?.dlurl;
    if (!dlurl)
        return { ok: false, erro: 'url_nao_encontrada', mensagem: '❌ Não foi possível obter o áudio do Spotify.' };
    const buffer = await baixarBuffer(dlurl);
...
export async function soundcloudDL(url) {
    ...
    const dlurl = res.data?.resposta?.dlurl;
    if (!dlurl)
        return { ok: false, erro: 'url_nao_encontrada', mensagem: '❌ Não foi possível obter o áudio do SoundCloud.' };
    const buffer = await baixarBuffer(dlurl);
...
export async function facebookDL(url) {
    ...
    const dlurl = res.data?.resposta?.dlurl || res.data?.resposta?.url;
    if (!dlurl)
        return { ok: false, erro: 'url_nao_encontrada', mensagem: '❌ Não foi possível baixar do Facebook.' };
    const buffer = await baixarBuffer(dlurl);
    return { ok: true, buffer, titulo: 'Facebook Video' };
}
/** Pesquisa imagens no Pinterest */
export async function pinterestSearch(consulta) {
    return chamarAPI('/api/pesquisas/pinterest', { query: consulta });
}
/** Busca letras de música */
export async function buscarLetra(musica) {
    return chamarAPI('/api/pesquisa/letra', { query: musica });
}
/** Pesquisa APK Mods */
export async function pesquisarAPK(consulta) {
