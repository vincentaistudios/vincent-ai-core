...
function formatarResultadoLeitura(dados) {
    let tipo = '📝 Texto';
    let extra = '';
    if (ehURL(dados)) {
        tipo = '🔗 URL';
    ...
    else if (dados.startsWith('mailto:'))
        tipo = '📧 Email';
    else if (dados.startsWith('tel:'))
        tipo = '📞 Telefone';
    else if (dados.startsWith('WIFI:'))
        tipo = '📶 Wi-Fi';
    else if (dados.startsWith('BEGIN:VCARD'))
        tipo = '👤 Contato (vCard)';
    else if (/^[0-9]{8,}$/.test(dados))
        tipo = '📊 Código de Barras';
    return `✅ *QR CODE LIDO*\n\n🏷️ Tipo: ${tipo}\n\n📝 *Conteúdo:*\n${dados}${extra}`;
}
/** Gera um QR Code a partir de um texto e retorna o buffer da imagem. */
export async function gerarQRCode(texto, tamanho = CONFIG.TAMANHO) {
    if (!texto?.trim())
        return { ok: false, erro: 'texto_vazio' };
    if (texto.length > 2000)
        return { ok: false, erro: 'texto_muito_longo' };
    try {
        ...
        return { ok: false, erro: 'falha_geracao' };
    }
}
/** Lê um QR Code a partir de um Buffer de imagem ou URL. */
export async function lerQRCode(entrada) {
    try {
        let resposta;
        ...
        if (Array.isArray(resultado) && resultado[0]?.symbol?.[0]) {
            ...
            if (symbol.error)
                return { ok: false, erro: symbol.error };
            if (symbol.data)
                return { ok: true, dados: symbol.data, mensagem: formatarResultadoLeitura(symbol.data) };
        }
...
export function obterURLQRCode(texto, tamanho = CONFIG.TAMANHO) {
