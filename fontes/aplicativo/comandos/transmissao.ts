 ...
 */
export const executarTransmissao: FuncaoComando = async (ctx) => {
  const { 
    conexao, origem, argumentos, ehCriador, marca, prefixo, responder, 
    obterListaContatos, obterTipoConteudo, info 
  } = ctx;
  ...
  for (let i = 0; i < lista.length; i += 5) {
    ...
    for (const jid of lote) {
      try {
        ...
        if (jid.endsWith('@g.us')) totalGrupos++;
        else totalContatos++;
      } catch (erro) {
