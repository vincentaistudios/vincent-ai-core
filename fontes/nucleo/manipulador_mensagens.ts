...
import { groqDecidirRota, groqResponderTexto, groqResponderImagem, groqTranscrever } from '../servicos/fluxo_groq.js';
import { resolverModelo } from '../servicos/cliente_groq.js';
import { extrairTextoDocumento } from '../servicos/texto_documento.js';
...
const ultimasExecucoesIA = new Map<string, number>();

/**
 * Verifica se a IA pode ser executada novamente para este chat respeitando o intervalo mínimo.
 */
const podeExecutarIAGora = (chatId: string, intervaloMinMs: number): boolean => {
  const agora = Date.now();
  const ultima = ultimasExecucoesIA.get(chatId) || 0;
  if (agora - ultima < intervaloMinMs) return false;
 ...
 */
export const carregarManipulador = async (atualizacao: { messages: proto.IWebMessageInfo[], type: string }, conexao: WASocket) => {
  ...
  for (const info of atualizacao?.messages || []) {
    ...
    if (!estaChatPermitido(origem, configuracoesBot)) continue;

    const tipoMensagem = getContentType(info.message || {}) || '';
    if (!info.message || atualizacao.type === "append") continue;
     ...
     */
    const responder = async (texto: string, tipo: string = 'text', opcoes: any = {}) => {
      if (tipo === 'text') {
        opcoes.text = texto;
      }
      return conexao.sendMessage(origem, opcoes, { quoted: info as any });
    };
      ...
      if (ehMidiaIA) {
        ...
        try {
          ...
          if (tipoMensagem === 'imageMessage') {
            ...
            if (r.ok) await responder(r.texto!);
         ...
         if (estaFiltrado(origem)) continue;
