 ...
 */
function lerEstado(): Record<string, string> {
  try {
    if (!fs.existsSync(CAMINHO_ESTADO)) return {};
 ...
 */
function gravarEstado(novoEstado: Record<string, string>): void {
  try {
 ...
 */
export function definirModoChat(idChat: string, modo: string): boolean {
  const m = String(modo || '').toLowerCase().trim();
  if (!(m === 'on' || m === 'off' || m === 'trigger')) return false;
