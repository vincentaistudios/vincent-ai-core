 ...
 */
export async function extrairTextoDocumento({ buffer, tipoMime, nomeArquivo }: OpcoesExtracao = {}) {
  const buf = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer || []);
  const mime = String(tipoMime || '').toLowerCase().trim();
  const nome = String(nomeArquivo || '').toLowerCase();

  if (!buf.length) return { ok: false, erro: 'vazio' };
