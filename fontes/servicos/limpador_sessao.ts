 ...
 */
function correspondeAPadrao(nome: string, padroes: string[]): boolean {
  const n = String(nome || '');
  for (const p of padroes) {
    if (!p) continue;
    if (p === n) return true;
    if (p.startsWith('*') && n.endsWith(p.slice(1))) return true;
    if (p.endsWith('*') && n.startsWith(p.slice(0, -1))) return true;
  }
  return false;
}
 ...
 */
function obterStatusSeguro(caminho: string): fs.Stats | null {
  try {
 ...
 */
export function limparDiretorioSessao(diretorioSessao: string, opcoes: OpcoesLimpeza = {}) {
  const habilitado = opcoes.habilitado ?? (process.env.SESSION_CLEAN_ENABLED === '1' || true);
  if (!habilitado) return { ok: true, pulado: true, deletados: 0, mantidos: 0 };
  ...
  const entradas: { nome: string; completo: string; mtimeMs: number }[] = [];
  ...
  const conjuntoManter = new Set<string>();
  ...
  let deletados = 0;
  for (const e of entradas) {
    if (conjuntoManter.has(e.nome)) continue;
    try {
