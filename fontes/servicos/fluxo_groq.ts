import { 
  obterConfiguracaoModelosGroq, 
  groqConversarComChave, 
  groqTranscreverComChave 
} from './cliente_groq.js';
...
const chaveRuimAte = new Map<string, number>();

/**
 * Converte um buffer de imagem em uma URL de dados (Base64).
 */
function converterBufferParaDataUrl(buffer: Buffer, tipoMime: string): string {
 ...
 */
function ehChaveRuim(idChave: string, agoraMs: number): boolean {
 ...
 */
function marcarChaveRuim(idChave: string, ms: number = 60000): void {
 ...
 */
function escolherChave(agora: Date = new Date()): { ok: true, idChave: string, chaveApi: string } | { ok: false, motivo: string, retryAfterSec?: number } {
  const pool = obterPoolChavesGroq();
  if (pool.length === 0) return { ok: false, motivo: 'sem_chaves' };
  ...
  for (let i = 0; i < pool.length; i++) {
    ...
    if (!k) continue;
    if (ehChaveRuim(k.id, agoraMs)) continue;
    ...
    if (typeof resultado.retryAfterSec === 'number') {
      if (!melhorRetry || resultado.retryAfterSec < melhorRetry.retryAfterSec) {
        melhorRetry = { motivo: (resultado as any).reason || 'rate_limit', retryAfterSec: resultado.retryAfterSec };
      }
    }
  }

  if (melhorRetry) return { ok: false, motivo: melhorRetry.motivo, retryAfterSec: melhorRetry.retryAfterSec };
 ...
 */
async function chamarComChaveDoPool({ userId, operacao, parametros, agora = new Date() }: any) {
  const pool = obterPoolChavesGroq();
  const maxTentativas = Math.max(1, pool.length);

  for (let i = 0; i < maxTentativas; i++) {
    const escolhida = escolherChave(agora);
    if (!escolhida.ok) return { ok: false, erro: escolhida.motivo || 'sem_chaves', retryAfterSec: escolhida.retryAfterSec };
 ...
 */
export async function groqDecidirRota({ userId, chatId, texto, temMedia, ehGrupo }: any) {
  ...
  if (!r.ok) return { ok: false, erro: (r as any).erro };

  try {
    const objeto = JSON.parse(String((r as any).conteudo || '').trim());
 ...
 */
export async function groqResponderTexto({ userId, prompt, promptSistema, modelo, usarBusca = false }: any) {
  const modelos = obterConfiguracaoModelosGroq();
  ...
  if (!r.ok) return { ok: false, erro: (r as any).erro };
  return { ok: true, texto: String((r as any).conteudo || '').trim() };
}

/**
 * Responde a uma mensagem com imagem (VLM).
 */
export async function groqResponderImagem({ userId, prompt, bufferImagem, tipoMime }: any) {
  const modelos = obterConfiguracaoModelosGroq();
 ...
 */
export async function groqTranscrever({ userId, bufferArquivo, nomeArquivo, tipoMime }: any) {
  const modelos = obterConfiguracaoModelosGroq();
  const r = await chamarComChaveDoPool({ 
      userId, 
  ...
  if (!r.ok) return { ok: false, erro: (r as any).erro };
  return { ok: true, texto: String((r as any).texto || '').trim() };
}
