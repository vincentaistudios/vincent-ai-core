...
type Token = { tipo: 'numero'; valor: number } | { tipo: 'funcao' | 'operador' | 'lparen' | 'rparen' | 'virgula'; valor: string };
type No = { tipo: 'numero'; valor: number } | { tipo: 'unario'; operador: string; operando: No } | { tipo: 'binario'; operador: string; esquerda: No; direita: No } | { tipo: 'funcao'; nome: string; args: No[] };
...
function tokenizar(expr: string): Token[] {
  ...
  while (i < expr.length) {
    ...
    if (/[0-9.]/.test(c)) {
      ...
      if (CONSTANTES[low] !== undefined) tokens.push({ tipo: 'numero', valor: CONSTANTES[low]! });
      else if (FUNCOES[low]) tokens.push({ tipo: 'funcao', valor: low });
      else throw new Error(`Função ou constante desconhecida: ${id}`);
    } else if (['+','-','*','/','%','^'].includes(c)) { tokens.push({ tipo: 'operador', valor: c }); i++; }
    else if (c === '(') { tokens.push({ tipo: 'lparen', valor: '(' }); i++; }
    else if (c === ')') { tokens.push({ tipo: 'rparen', valor: ')' }); i++; }
    else if (c === ',') { tokens.push({ tipo: 'virgula', valor: ',' }); i++; }
    else if (/\s/.test(c)) i++;
...
function parsear(tokens: Token[]): No {
  let pos = 0;
  ...
  const expressao = (): No => {
    let esq = termo();
    while (peek()?.tipo === 'operador' && ['+','-'].includes((peek() as any).valor)) {
      const op = (consumir() as any).valor;
  ...
  const potencia = (): No => {
    let esq = unario();
    if (peek()?.tipo === 'operador' && (peek() as any).valor === '^') {
      consumir();
  ...
  const unario = (): No => {
    if (peek()?.tipo === 'operador' && ['+','-'].includes((peek() as any).valor)) {
      const op = (consumir() as any).valor;
  ...
  const fator = (): No => {
    ...
    if (token.tipo === 'numero') { consumir(); return { tipo: 'numero', valor: (token as any).valor }; }
    if (token.tipo === 'funcao') {
      const nome = (consumir() as any).valor;
      if (peek()?.tipo !== 'lparen') throw new Error(`Esperado '(' após ${nome}`);
      consumir();
      const args: No[] = [];
      if (peek()?.tipo !== 'rparen') {
        args.push(expressao());
        while (peek()?.tipo === 'virgula') { consumir(); args.push(expressao()); }
      }
      if (peek()?.tipo !== 'rparen') throw new Error(`Esperado ')' após ${nome}`);
      consumir();
    ...
    if (token.tipo === 'lparen') {
      ...
      if (peek()?.tipo !== 'rparen') throw new Error("Esperado ')'");
      consumir();
      return expr;
    }
    throw new Error(`Token inesperado: ${(token as any).valor}`);
  };

  const resultado = expressao();
  if (pos < tokens.length) throw new Error(`Token inesperado: ${(tokens[pos] as any)?.valor}`);
  return resultado;
}

function avaliar(no: No): number {
  if (no.tipo === 'numero') return no.valor;
  if (no.tipo === 'unario') { const v = avaliar(no.operando); return no.operador === '-' ? -v : v; }
  if (no.tipo === 'binario') {
    const e = avaliar(no.esquerda), d = avaliar(no.direita);
    switch (no.operador) {
      case '+': return e + d; case '-': return e - d;
      case '*': return e * d; case '/': if (d === 0) throw new Error('Divisão por zero'); return e / d;
      case '%': return e % d; case '^': return Math.pow(e, d);
    }
  }
  if (no.tipo === 'funcao') {
    const fn = FUNCOES[no.nome];
    if (!fn) throw new Error(`Função desconhecida: ${no.nome}`);
...
export function calcular(expressao: string) {
  if (!expressao?.trim()) return { ok: false, erro: 'expressao_vazia' };
  const expr = expressao.trim()
    .replace(/×/g, '*').replace(/÷/g, '/').replace(/,/g, '.').replace(/\*\*/g, '^');
  if (expr.length > 200) return { ok: false, erro: 'expressao_muito_longa' };
  try {
    const resultado = avaliar(parsear(tokenizar(expr)));
    if (!isFinite(resultado)) return { ok: true, resultado: resultado > 0 ? '∞' : '-∞', expressao: expr };
...
export function converter(valor: number, de: string, para: string) {
  ...
  if (!fn) return { ok: false, erro: 'conversao_nao_suportada' };
