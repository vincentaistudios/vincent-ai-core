import { generateWAMessageFromContent, proto, WASocket } from '@whiskeysockets/baileys';

/**
 * Envia botões de resposta rápida (Quick Reply).
 */
export async function enviarBotoesRapidos(conexao: WASocket, jid: string, titulo: string, texto: string, rodape: string, botoesArray: { text: string, id: string }[]) {
 ...
 */
export async function enviarBotoesDeAcao(conexao: WASocket, jid: string, texto: string, urlSite: string, textoCopia: string) {
    const msg = generateWAMessageFromContent(jid, {
        viewOnceMessage: {
            message: {
                messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
 ...
 */
export async function enviarMenuLista(conexao: WASocket, jid: string, textoPrincipal: string, tituloBotao: string, secoes: any[]) {
    const msg = generateWAMessageFromContent(jid, {
        viewOnceMessage: {
            message: {
                messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
 ...
 */
export async function enviarCarrossel(conexao: WASocket, jid: string, textoCorpo: string, cardsArray: any[]) {
    const cards = cardsArray.map(card => {
        return {
            ...
            nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
                buttons: (card.botoes || []).map((btn: any) => ({
                    name: "quick_reply",
 ...
 */
export async function enviarEnquete(conexao: WASocket, jid: string, nome: string, valores: string[], multiplasEscolhas = false) {
 ...
 */
export async function enviarContatoCard(conexao: WASocket, jid: string, nomeContato: string, numeroContato: string) {
 ...
 */
export async function solicitarLocalizacao(conexao: WASocket, jid: string, texto: string) {
    const msg = generateWAMessageFromContent(jid, {
        viewOnceMessage: {
            message: {
                messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
