...
const timestampsRpmPorChave = new Map<string, number[]>();

/**
 * Retorna o caminho do arquivo de persistência de uso.
 */
function obterCaminhoUso(): string {
  return String(process.env.GROQ_USAGE_PATH || CAMINHO_USO_PADRAO);
}

function obterChaveHoje(data: Date = new Date()): string {
  return data.toISOString().slice(0, 10);
}

function obterChaveMes(data: Date = new Date()): string {
  return data.toISOString().slice(0, 7);
}

function lerJson(caminho: string, valorPadrao: any) {
  try {
...
function escreverJson(caminho: string, valor: any): boolean {
  try {
...
function lerArmazenamento() {
  ...
  if (!st || typeof st !== 'object') return { versao: 1, chaves: {} as any };
  if (!st.chaves || typeof st.chaves !== 'object') st.chaves = {};
  if (!st.versao) st.versao = 1;
  return st;
}

function gravarArmazenamento(st: any) {
  return escreverJson(obterCaminhoUso(), st);
}

function garantirChave(st: any, idChave: string) {
 ...
 */
function limparRpmAntigo(idChave: string, agoraMs: number): number[] {
 ...
 */
export function podeUsarChave(idChave: string, agora: Date = new Date()) {
 ...
 */
export function registrarUso(idChave: string, agora: Date = new Date()) {
  const agoraMs = agora.getTime();
  const listaRpm = limparRpmAntigo(idChave, agoraMs);
 ...
 */
export function obterEstatisticasChave(idChave: string, agora: Date = new Date()) {
  const agoraMs = agora.getTime();
  const listaRpm = limparRpmAntigo(idChave, agoraMs);
  const armazem = lerArmazenamento();
  const chave = garantirChave(armazem, idChave);
 ...
 */
export function obterTodasEstatisticas(agora: Date = new Date()) {
  const armazem = lerArmazenamento();
  const resultado = [];
