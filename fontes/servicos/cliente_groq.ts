...
export const APELIDOS_MODELOS: Record<string, string> = {
 ...
 */
export function resolverModelo(apelidoOuId: string): string {
 ...
 */
function resolverConfiguracao(sobreposicoes: any = {}) {
 ...
 */
export async function listarModelosDisponiveis(chaveApi: string) {
  const cfg = resolverConfiguracao({ chaveApi });
  if (!cfg.chaveApi) return { ok: false, erro: 'CHAVE_API_AUSENTE' };
  ...
  try {
    ...
    if (!resposta.ok) throw new Error(`ERRO_HTTP_${resposta.status}`);
 ...
 */
export async function groqConversarComChave({ 
  chaveApi, urlBase, tempoEsgotadoMs, mensagens, modelo, temperatura = 0.2, maxTokens 
}: any = {}) {
  ...
  try {
    ...
    let dados = null;
    try { dados = textoBruto ? JSON.parse(textoBruto) : null } catch { dados = null }

    if (!resposta.ok) {
      const erro: any = new Error('ERRO_API_GROQ');
 ...
 */
export async function groqConversar(opcoes: any = {}) {
 ...
 */
export async function groqTranscreverComChave({ 
  chaveApi, urlBase, tempoEsgotadoMs, bufferArquivo, nomeArquivo, tipoMime, modelo 
}: any = {}) {
  const cfg = resolverConfiguracao({ chaveApi, urlBase, tempoEsgotadoMs });
  if (!cfg.chaveApi) {
    throw new Error('CHAVE_API_GROQ_AUSENTE');
